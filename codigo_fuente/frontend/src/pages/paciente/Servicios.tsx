...
function nombreProfesional(p: ProfesionalAPI): string {
  ...
  if (p.nombreCompleto && p.nombreCompleto.trim()) return p.nombreCompleto.trim();
...
const Servicios: React.FC = () => {
    ...
    async function fetchData() {
      ...
      try {
        const [rPro, rEsp] = await Promise.all([
          api.get("/odontologos/", { params: { estado: true, page_size: 1000 } }),
          api.get("/especialidades/", { params: { estado: true, page_size: 1000 } }),
        ]);

        if (!isMounted) return;

        const pros = Array.isArray(rPro.data) ? rPro.data : rPro.data?.results || [];
        const esps = Array.isArray(rEsp.data) ? rEsp.data : rEsp.data?.results || [];
    ...
    const nombres = uniqStrings(
      profesionales.flatMap((p) => {
        ...
        if (p.especialidades_detalle?.length)
          return p.especialidades_detalle.map((d) => d.nombre || "").filter(Boolean);
        return [];
      })
    );
  ...
  const listaFiltrada = useMemo(() => {
    ...
    return profesionales.filter((p) => {
      ...
      if (espSel !== "todas") {
        const target = opcionesEspecialidad.find((e) => e.id_especialidad === espSel)?.nombre;
        if (target) {
          const plano = (p.especialidades || []).map((x) => (x || "").toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <div className="rounded-xl bg-white shadow px-4 py-2">
          ...
          <div className="text-lg font-semibold text-gray-800">{profesionales.length}</div>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white shadow p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="col-span-2">
            <label className="text-xs text-gray-600 mb-1 block">Buscar por nombre</label>
          ...
          <div>
            <label className="text-xs text-gray-600 mb-1 block">Filtrar por especialidad</label>
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2" size={18} />
              <select
                className="w-full rounded-lg border pl-9 pr-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                value={espSel}
                onChange={(e) =>
                  setEspSel(e.target.value === "todas" ? "todas" : Number(e.target.value))
                }
              >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {listaFiltrada.map((p) => {
            ...
            const detOrdenadas =
              [...(p.especialidades_detalle ?? [])].sort((a, b) =>
            ...
            return (
              <article
                key={p.id_odontologo}
                className="rounded-2xl bg-white shadow hover:shadow-md transition-shadow p-4 flex flex-col"
              >
                {/* Header */}
                <div className="flex items-center gap-4">
                  {foto ? (
                    ...
                    <div className="h-16 w-16 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-bold">
                      {iniciales(p)}
                    </div>
                  )}
                ...
                {p.consultorio && (p.consultorio.nombre || p.consultorio.numero) && (
                  <div className="mt-2 text-sm text-gray-700 flex items-center gap-2">
                    <Building2 size={16} />
                    <span className="truncate">
                      {p.consultorio.nombre || "Consultorio"}{" "}
                      {p.consultorio.numero ? `• Nº ${p.consultorio.numero}` : ""}
                    </span>
                  </div>
                )}
